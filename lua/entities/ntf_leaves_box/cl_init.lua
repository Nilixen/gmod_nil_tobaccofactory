...
function ENT:Draw()
	...
	if self:GetPos():Distance( LocalPlayer():GetPos()) < 400 then
		cam.Start3D2D(self:GetPos()+ang:Up()*4.2,ang,0.03)
			...
			TobaccoFactory:drawBoxCorners(-190,-140,380,250,25,6,2,DisColor)

			draw.SimpleText(TobaccoFactory.Config.Lang.LeavesBox,"ntf_very_small",0,-100,Color(255,255,255,DisColor),1,0)

			if self:GetDoneTime() == 1 and IsValid(self:GetParent()) and self:GetParent():GetClass() == "ntf_drying_shelf" then
				draw.SimpleText(TobaccoFactory.Config.Lang.TakeOut,"ntf_very_small",0,-20,Color(255,255,255,DisColor),1,0)

			elseif self:GetDoneTime() == 1 and !IsValid(self:GetParent()) then
			...
			elseif IsValid(self:GetParent()) and self:GetParent():GetClass() == "ntf_drying_shelf" then
				draw.SimpleText(TobaccoFactory.Config.Lang.Wait.." "..(self:GetDoneTime()-1).." "..TobaccoFactory.Config.Lang.Seconds,"ntf_extreme_small",0,-20,Color(255,255,255,DisColor),1,0)
