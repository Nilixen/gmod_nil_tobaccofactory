...
function ENT:Draw()
	...
	if self:GetPos():Distance( LocalPlayer():GetPos()) < 400 then
		cam.Start3D2D(self:GetPos()+ang:Up()*12.3,ang,0.03)
			...
			draw.SimpleText(TobaccoFactory.Config.Lang.LeavesPack,"ntb_big",0,-200,Color(255,255,255,DisColor),1,0)
			draw.SimpleText(self:GetBoxes().." "..TobaccoFactory.Config.Lang.packsleft,"ntb_big",0,-80,Color(255,255,255,DisColor),1,0)
			draw.SimpleText(TobaccoFactory.Config.Lang.Unpack,"ntb_small",0,50,Color(255,255,255,DisColor),1,0)
		cam.End3D2D()
	end
